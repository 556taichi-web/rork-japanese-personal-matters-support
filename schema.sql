...
-- Create summaries table (daily/weekly AI-generated summaries)
CREATE TABLE public.summaries (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
  summary_type TEXT CHECK (summary_type IN ('daily', 'weekly', 'monthly')) NOT NULL,
  date DATE NOT NULL,
  content JSONB NOT NULL,
  ai_recommendations TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(user_id, summary_type, date)
);

-- Create indexes for better performance
CREATE INDEX idx_workouts_user_id_date ON public.workouts(user_id, date DESC);
CREATE INDEX idx_workout_items_workout_id ON public.workout_items(workout_id);
CREATE INDEX idx_nutrition_logs_user_id_date ON public.nutrition_logs(user_id, date DESC);
CREATE INDEX idx_nutrition_targets_user_id_date ON public.nutrition_targets(user_id, date);
CREATE INDEX idx_summaries_user_id_type_date ON public.summaries(user_id, summary_type, date DESC);
